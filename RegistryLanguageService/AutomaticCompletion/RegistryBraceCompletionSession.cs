...
namespace RegistryLanguageService.AutomaticCompletion
{
    [Export(typeof(IBraceCompletionSessionProvider))]
    [ContentType(RegistryContentTypeNames.Registry)]
    [BracePair(RegistrySyntaxFacts.SectionNameOpeningBracket, RegistrySyntaxFacts.SectionNameClosingBracket)]
    [BracePair(RegistrySyntaxFacts.Quote, RegistrySyntaxFacts.Quote)]
    [BracePair(RegistrySyntaxFacts.TypeSpecifierOpeningBrace, RegistrySyntaxFacts.TypeSpecifierClosingBrace)]
    internal sealed class RegistryBraceCompletionSessionProvider : IBraceCompletionSessionProvider
    {
...
#pragma warning restore 649
        ...
        public bool TryCreateSession(ITextView textView, SnapshotPoint openingPoint, char openingBrace, char closingBrace, out IBraceCompletionSession session)
        {
            ...
            if (openingBrace == RegistrySyntaxFacts.SectionNameOpeningBracket)
            {
                ...
                {
                    session = new RegistryBraceCompletionSession(
            ...
            else
            {
                RegistryPropertySyntax property = root.Sections
                    ...
                    .FirstOrDefault(p => p.Span.IntersectsWith(line.Extent));

                if (openingBrace == RegistrySyntaxFacts.Quote)
                {
                    if (property == null)
                    {
                        session = new RegistryBraceCompletionSession(
                ...
                else if (openingBrace == RegistrySyntaxFacts.TypeSpecifierOpeningBrace)
                {
                    if (property != null)
                    {
                        if (!property.TypeToken.IsMissing &&
                            property.TypeSpecifierOpeningBraceToken.IsMissing &&
                            openingPoint == property.TypeToken.Span.Span.End)
                        {
                            session = new RegistryBraceCompletionSession(
                                textView, openingPoint,
                                openingBrace, closingBrace,
        ...
        private sealed class RegistryBraceCompletionSession : IBraceCompletionSession
        {
